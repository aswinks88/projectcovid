let useState,axios,cheerio,fs,Path,https,url,xlsx,xlreader,xltojson;_da1‍.x([["getHTML",()=>getHTML],["findCovid19TotalCases",()=>findCovid19TotalCases],["casesbyDHB",()=>casesbyDHB],["findCovidDataOvertheTime",()=>findCovidDataOvertheTime],["fetchRecoveryData",()=>fetchRecoveryData],["covid19TotalCount",()=>covid19TotalCount],["TotalCasesbyDHB",()=>TotalCasesbyDHB],["covid19TotalOverTime",()=>covid19TotalOverTime],["recoveryDataCount",()=>recoveryDataCount]]);_da1‍.w("react",[["useState",["useState"],function(v){useState=v}]]);_da1‍.w("axios",[["default",["axios"],function(v){axios=v}]]);_da1‍.w("cheerio",[["default",["cheerio"],function(v){cheerio=v}]]);_da1‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_da1‍.w("path",[["default",["Path"],function(v){Path=v}]]);_da1‍.w("https",[["default",["https"],function(v){https=v}]]);_da1‍.w("url",[["default",["url"],function(v){url=v}]]);_da1‍.w("xlsx",[["default",["xlsx"],function(v){xlsx=v}]]);_da1‍.w("read-excel-file/node",[["default",["xlreader"],function(v){xlreader=v}]]);_da1‍.w("convert-excel-to-json",[["default",["xltojson"],function(v){xltojson=v}]]);
 ...
 //scraping the download link from another page but from the same site
    ...
    const fileWrite = fs.createWriteStream(path)
     fs.readdir(pathtoFiles,(err, existingfile) => {
        _da1‍.g.console.log(pathtoFiles)

          if(filename === existingfile[0]){
              _da1‍.g.console.log(pathtoFiles + '/\/' +existingfile[0])
              fs.unlink(pathtoFiles + existingfile[0], err => {if(err) throw err 
            ...
            https.get(downloadUrl, res => {
                res.on('data', data => {
                    ...
                    fileWrite.end()
                    _da1‍.g.console.log(filename + ' downloaded to ' + path)
                })
            })
          }
    })
