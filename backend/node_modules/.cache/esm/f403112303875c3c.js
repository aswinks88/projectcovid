let axios,cheerio,fs,Path,https,url,xlsx;_038‍.x([["getHTML",()=>getHTML],["findCovid19TotalCases",()=>findCovid19TotalCases],["casesbyDHB",()=>casesbyDHB],["findCovidDataOvertheTime",()=>findCovidDataOvertheTime],["fetchRecoveryData",()=>fetchRecoveryData],["covid19TotalCount",()=>covid19TotalCount],["TotalCasesbyDHB",()=>TotalCasesbyDHB],["covid19TotalOverTime",()=>covid19TotalOverTime],["recoveryDataCount",()=>recoveryDataCount]]);_038‍.w("axios",[["default",["axios"],function(v){axios=v}]]);_038‍.w("cheerio",[["default",["cheerio"],function(v){cheerio=v}]]);_038‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_038‍.w("path",[["default",["Path"],function(v){Path=v}]]);_038‍.w("https",[["default",["https"],function(v){https=v}]]);_038‍.w("url",[["default",["url"],function(v){url=v}]]);_038‍.w("xlsx",[["default",["xlsx"],function(v){xlsx=v}]]);
...
const URL = {currentcases: 'https://www.health.govt.nz/our-work/diseases-and-conditions/covid-19-novel-coronavirus/covid-19-current-situation/covid-19-current-cases',
 ...
 //scraping the download link from another page but from the same site
    ...
    const fileWrite = fs.writeFileSync('./cases.json')
    https.get(downloadUrl, res => {
        res.on('data', data => {
            const filedata = xlsx.read(data)
            const spreadsheet = filedata.SheetNames.map(
                spsheet => {
                    return{
                        spsheet, data: xlsx.utils.sheet_to_json(filedata.Sheets[spsheet])
                    }
                }
            )
            _038‍.g.console.log(spreadsheet)
            fileWrite.write(spreadsheet)
        })
    })

    _038‍.g.console.log(1, filename)
...
// })
    ...
    // .then(response => {
    //     const readXlsx = xlsx.read(response.data)
    //     readXlsx.
